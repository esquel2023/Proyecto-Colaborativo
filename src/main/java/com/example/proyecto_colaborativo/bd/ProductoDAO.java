...
public class ProductoDAO {
    ...
    // Dentro de tu clase ProductoDAO:
    public static void modificar(Producto producto) throws SQLException {
        String query = "UPDATE Producto SET nombre = ?, cantidad = ?, precio = ?, codigodebarra = ?, activado = ? WHERE idproducto = ?";

        try (Connection c = Database.getConnection(); // Ajustá a tu método de conexión
             PreparedStatement ps = c.prepareStatement(query)) {

            ps.setString(1, producto.getNombre());
            ps.setInt(2, producto.getCantidad());
            ps.setDouble(3, producto.getPrecio());
            ps.setString(4, producto.getCodigoBarra());
            ps.setBoolean(5, producto.isActivado()); // Acá guardará el 'true' al reactivarlo
            ps.setInt(6, producto.getidProducto()); // Tu método con 'd' minúscula
